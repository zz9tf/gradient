...
def gen_instance_hv_map(ann, crop_shape):
    ...
    for inst_id in inst_list:
        ...
        H, W = fixed_ann.shape[:2]
        r0, r1, c0, c1 = inst_box
        ...
        c0 = int(np.clip(c0, 0, W))
        c1 = int(np.clip(c1, 0, W))

        if r1 - r0 < 2 or c1 - c0 < 2:
            raise ValueError(f"Instance box too small: {r1 - r0}x{c1 - c0}")

        inst_map = inst_map[r0:r1, c0:c1]
        ...
        inst_x_range = np.arange(1, inst_map.shape[1] + 1) - inst_com[1]
        inst_y_range = np.arange(1, inst_map.shape[0] + 1) - inst_com[0]
        ...
        ####
        x_map_box = x_map[r0:r1, c0:c1]
        y_map_box = y_map[r0:r1, c0:c1]
        mask = inst_map > 0
        x_map_box[mask] = inst_x[mask]        
        y_map_box[mask] = inst_y[mask]
