...
class FileLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            import os, traceback
            dump_dir = "/tmp/hovernet_bad_samples"
            os.makedirs(dump_dir, exist_ok=True)

            # 打印关键信息
            print("\n" + "=" * 90)
            print("[TargetGen ERROR]")
            print("idx:", idx)
            print("path:", path)
            print("inst_map shape:", inst_map.shape, "dtype:", inst_map.dtype)
            uniq = np.unique(inst_map)
            print("unique instance ids (first 50):", uniq[:50], "num_unique:", len(uniq))
            # dump
            np.save(os.path.join(dump_dir, f"inst_map_idx{idx}.npy"), inst_map)
            print("Saved:", os.path.join(dump_dir, f"inst_map_idx{idx}.npy"))
    ...
    def __get_augmentation(self, mode, rng):
        if mode == "train":
            ...
            # Same deterministic pipeline for validation and test: center crop only.
            shape_augs = [
                iaa.CropToFixedSize(
                    self.input_shape[0], self.input_shape[1], position="center"
                )
            ...
            raise ValueError("__get_augmentation: mode must be 'train', 'valid', or 'test', got %r" % mode)
