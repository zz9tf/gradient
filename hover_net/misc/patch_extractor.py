...
class PatchExtractor(object):
    ...
    def __extract_valid(self, x):
        ...
        # --- FIX: if image is smaller than window, there is no valid patch ---
        if im_h < win_h or im_w < win_w:
            return []
    ...
    def __extract_mirror(self, x):
        ...
        # 2) if still smaller than win_size, center-pad to win_size
        H, W = x.shape[:2]
        need_h = max(0, self.win_size[0] - H)
        need_w = max(0, self.win_size[1] - W)
        if need_h > 0 or need_w > 0:
            ...
            # split into RGB + label to pad differently
            if x.shape[2] >= 3:
                rgb = x[..., :3]
                lab = x[..., 3:]

                rgb = np.pad(rgb, ((top, bot), (left, right), (0, 0)), mode="reflect")
                if lab.shape[2] > 0:
                    lab = np.pad(lab, ((top, bot), (left, right), (0, 0)),
                                mode="constant", constant_values=0)
                    x = np.concatenate([rgb, lab], axis=-1)
                else:
                    x = rgb
            else:
                raise ValueError("x must have at least 3 channels")
