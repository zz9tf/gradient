...
from cell_segmentation.datasets.monusac_patched import MoNuSACPatchedDataset
...
def select_dataset(
    ...
    """Select a cell segmentation dataset from the provided ones.

    Args:
        dataset_name (str): Name of dataset to use.
            Must be one of: [pannuke, conic, monusac_patched]
    ...
    elif dataset_name.lower() == "monusac_patched":
        dataset = MoNuSACPatchedDataset(
            dataset_path=dataset_config["dataset_path"],
            split=split,
            transforms=transforms,
            stardist=dataset_config.get("stardist", False),
            ...
            val_split=dataset_config.get("val_split", "valid"),
            test_split=dataset_config.get("test_split", "test"),
            patch_subdir=dataset_config.get("patch_subdir", "256x256_164x164"),
        )
    else:
